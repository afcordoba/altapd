...
//	private static final String ICodLoteProp = "";
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		DatabaseUtility dbu = null;
		dbu = new DatabaseUtility();
		List<PlantaPorEncargado> plantasXEncargado=null;
		try {
			plantasXEncargado = dbu.getPlantasUsuario(user.getId_encargado(), user.getCampaña());
		...
		if ("mostrarRegLluvia".equalsIgnoreCase((String) request.getParameter("accion"))) { //Localidades por encargado para registro de lluvias
			...
			List<RegistroLluvia> registros = null;
			try {
				registros = dbu.getRegistroLluviaEncargado(user.getId_encargado(), fechaIngresada);
			} catch ( SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ("busquedaCodigo".equalsIgnoreCase((String) request.getParameter("accion"))) {
			...
			java.math.BigDecimal IPorHumCos = null;
			...
			ZscaDatosPlanificacion[] planificaciones = soap.ZCa0011ListaPlanificacion(IAtributo, IAvanceCosecha, ICampana, ICodCampoProp, ICodCampoTerc, ICodLoteProp, ICodLoteTerc, ICodMatSembrado, IContratistaCose, ICultivo, IEncargado, IIdPlanificacion, IIdSector, INombreCliente, IPorHumCos, IPropTercero, IProveedor, ISociedad); 
...
/*			items[1].setMandt("0");
		...
		if ("altaRegistroLluvia".equalsIgnoreCase((String) request.getParameter("accion"))) {
			...
			int exitoso = 0;
			try {
				exitoso = dbu.saveRegistroLluvia(encargado, provincia, ciudad, mil, fecha, exitoso);
			} catch ( SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		if ("modificarRegistroLluvia".equalsIgnoreCase((String) request.getParameter("accion"))) {
			...
			int exitoso = 0;
			try {
				exitoso = dbu.updateRegistroLluvia(encargado,ciudad, mil, fecha, exitoso);
			} catch ( SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
